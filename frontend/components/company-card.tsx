...
import { Leaf, Users, Shield, MapPin, ExternalLink, Calendar, Heart } from "lucide-react"
        ...
        <CardHeader className="pb-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start justify-between">
              <h3 className="text-xl font-semibold text-gray-900 leading-tight flex-1 pr-2">{company.brand_name}</h3>
              {user && (
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  <Heart
                    className={`h-4 w-4 ${
                      isCompanyFavorite ? "fill-red-500 text-red-500" : "text-gray-400 hover:text-red-500"
                    }`}
                  />
                </Button>
              )}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant={getScoreBadgeVariant(company.overall_score)} className="text-base px-3 py-1">
                  {company.overall_score}
                </Badge>
                <span className="text-xs text-gray-500">{getScoreLabel(company.overall_score)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-sm font-medium ${getScoreColor(company.environmental_score)}`}>
                  {company.environmental_score}/100
                </span>
              </div>
              <Progress value={company.environmental_score} className="h-2" />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-sm font-medium ${getScoreColor(company.social_score)}`}>
                  {company.social_score}/100
                </span>
              </div>
              <Progress value={company.social_score} className="h-2" />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className={`text-sm font-medium ${getScoreColor(company.governance_score)}`}>
                  {company.governance_score}/100
                </span>
              </div>
              <Progress value={company.governance_score} className="h-2" />
            </div>
          </div>
